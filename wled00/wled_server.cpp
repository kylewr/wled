...
#if defined(WLED_LOCKED_LED_PINS) || defined(WLED_LOCKED_LED_BUS_COUNT)
static const char s_led_pins_locked[] PROGMEM = "LED output configuration is locked by this firmware build.";
#endif
...
// Returns true when every product-reserved LED pin remains in a settings form.
static bool requestContainsLockedLedPins(AsyncWebServerRequest *request) {
  for (uint8_t lockedPin : lockedLedPins) {
    bool found = false;
    for (uint8_t bus = 0; bus < 36 && !found; bus++) {
      int offset = bus < 10 ? '0' : 'A' - 10;
      for (uint8_t pin = 0; pin < OUTPUT_MAX_PINS; pin++) {
        char arg[4] = "L0";
        arg[1] = '0' + pin;
        arg[2] = offset + bus;
        if (!request->hasArg(arg)) break;
        if (request->arg(arg).toInt() == lockedPin) {
#ifdef WLED_HIDDEN_STATUS_LED_PINS
          char skipArg[4] = "SL";
          skipArg[2] = offset + bus;
          // The skipped physical LED is the status pixel and must remain reserved.
          if (!request->hasArg(skipArg) || request->arg(skipArg).toInt() < 1) continue;
#endif
          found = true;
          break;
        }
...
// Returns true when every product-reserved LED pin remains in a JSON config update.
static bool configContainsLockedLedPins(JsonObject hw) {
  JsonObject led = hw[F("led")];
  JsonArray outputs = led["ins"];
  if (led.isNull() || outputs.isNull()) return false;

  for (uint8_t lockedPin : lockedLedPins) {
    bool found = false;
    for (JsonObject output : outputs) {
      JsonArray pins = output["pin"];
      for (int pin : pins) {
        if (pin == lockedPin) {
#ifdef WLED_HIDDEN_STATUS_LED_PINS
          // The skipped physical LED is the status pixel and must remain reserved.
          if ((output["skip"] | 0) < 1) continue;
#endif
          found = true;
          break;
        }
...
// AI: below section was generated by an AI
// Returns true when the submitted form keeps the product's fixed number of LED outputs.
static bool requestContainsLockedLedBusCount(AsyncWebServerRequest *request) {
  uint8_t busCount = 0;
  for (; busCount < 36; busCount++) {
    int offset = busCount < 10 ? '0' : 'A' - 10;
    char arg[4] = "L0";
    arg[2] = offset + busCount;
    if (!request->hasArg(arg)) break;
  }
  return busCount == WLED_LOCKED_LED_BUS_COUNT;
}

// Returns true when the JSON config keeps the product's fixed number of LED outputs.
static bool configContainsLockedLedBusCount(JsonObject hw) {
  JsonObject led = hw[F("led")];
  JsonArray outputs = led["ins"];
  return !led.isNull() && !outputs.isNull() && outputs.size() == WLED_LOCKED_LED_BUS_COUNT;
}
// AI: end
#endif
...
#if defined(WLED_LOCKED_LED_PINS) || defined(WLED_LOCKED_LED_BUS_COUNT)
  // A cfg.json restore can replace LED output definitions, so reject it as a whole.
  if (filename.indexOf(F("cfg.json")) >= 0) {
    if (index == 0) request->send(403, FPSTR(CONTENT_TYPE_PLAIN), FPSTR(s_led_pins_locked));
    return;
  }
#endif
  if (!index) {
...
#if defined(WLED_LOCKED_LED_PINS) || defined(WLED_LOCKED_LED_BUS_COUNT)
      JsonObject hw = root[F("hw")];
      bool invalidLedConfig = false;
#ifdef WLED_LOCKED_LED_PINS
      invalidLedConfig = !configContainsLockedLedPins(hw);
#endif
#ifdef WLED_LOCKED_LED_BUS_COUNT
      invalidLedConfig = invalidLedConfig || !configContainsLockedLedBusCount(hw);
#endif
      if (!hw.isNull() && hw.containsKey(F("led")) && invalidLedConfig) {
        releaseJSONBufferLock();
        serveJsonError(request, 403, ERR_DENIED);
        return;
      }
#endif
...
#if defined(WLED_LOCKED_LED_PINS) || defined(WLED_LOCKED_LED_BUS_COUNT)
    bool invalidLedSettings = false;
#ifdef WLED_LOCKED_LED_PINS
    invalidLedSettings = !requestContainsLockedLedPins(request);
#endif
#ifdef WLED_LOCKED_LED_BUS_COUNT
    invalidLedSettings = invalidLedSettings || !requestContainsLockedLedBusCount(request);
#endif
    if (subPage == SUBPAGE_LEDS && invalidLedSettings) {
      serveMessage(request, 403, FPSTR(s_accessdenied), FPSTR(s_led_pins_locked), 254);
      return;
    }
#endif
