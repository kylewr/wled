...
const noDevBanner = process.env.WLED_NO_DEV_BANNER === "1";
const uiBuildMode = `WLED_UI_NO_DEV_BANNER=${noDevBanner ? 1 : 0}`;
...
// Removes only explicitly marked development-banner source from release UI builds.
function applyDevBannerOption(source) {
  if (!noDevBanner) return source;
  return source
    .replace(/<!-- DEV_BANNER_START -->[\s\S]*?<!-- DEV_BANNER_END -->\s*/g, "")
    .replace(/\/\* DEV_BANNER_START \*\/[\s\S]*?\/\* DEV_BANNER_END \*\/\s*/g, "")
    .replace(/\/\/ DEV_BANNER_START[\s\S]*?\/\/ DEV_BANNER_END\s*/g, "");
}
// AI: end
...
async function writeHtmlGzipped(sourceFile, resultFile, page, inlineCss = true) {
  ...
  inline.html({
    ...
    async function (error, html) {
      ...
      html = applyDevBannerOption(adoptVersionAndRepo(html));
...
async function specToChunk(srcDir, s) {
  ...
  if (s.method == "plaintext" || s.method == "gzip") {
    ...
    str = applyDevBannerOption(adoptVersionAndRepo(str));
...
function isAlreadyBuilt(webUIPath, packageJsonPath = "package.json") {
  ...
  if (!fs.readFileSync(output[0], "utf8").includes(uiBuildMode)) return false;
